...
/// Direct Swift/OpenGL ES port of EmeraldGLEffect.kt.
final class EmeraldGLEffect: GLEffect {
  ...
  private var vineProgram: GLuint = 0
  private var vinePosition: GLint = -1
  private var vineColor: GLint = -1
  private var vineCenterDistance: GLint = -1
  private var vineTrailDistance: GLint = -1
  private var vineTime: GLint = -1

  private var leafProgram: GLuint = 0
  private var leafPosition: GLint = -1
  private var leafUV: GLint = -1
  private var leafColor: GLint = -1

  private var ringProgram: GLuint = 0
  private var ringPosition: GLint = -1
  private var ringColor: GLint = -1
  ...
  private let leafFloats = GLFloatBuffer(capacity: 48 * 6 * 8)
  private let ringFloats = GLFloatBuffer(capacity: maxHeads * ringCount * ringSegments * 2 * 6)
  ...
  override func onGLReady(context: GLRenderContext) {
    vineProgram = GLHelper.buildProgram(
      vertexSource: GLEffectShaders.Emerald.vineVert,
      fragmentSource: GLEffectShaders.Emerald.vineFrag
    )
    vinePosition = glGetAttribLocation(vineProgram, "aPosition")
    vineColor = glGetAttribLocation(vineProgram, "aColor")
    vineCenterDistance = glGetAttribLocation(vineProgram, "aCenterDist")
    vineTrailDistance = glGetAttribLocation(vineProgram, "aTrailDist")
    vineTime = glGetUniformLocation(vineProgram, "uTime")

    leafProgram = GLHelper.buildProgram(
      vertexSource: GLEffectShaders.Emerald.leafVert,
      fragmentSource: GLEffectShaders.Emerald.leafFrag
    )
    leafPosition = glGetAttribLocation(leafProgram, "aPosition")
    leafUV = glGetAttribLocation(leafProgram, "aUV")
    leafColor = glGetAttribLocation(leafProgram, "aColor")

    ringProgram = GLHelper.buildProgram(
      vertexSource: GLEffectShaders.Emerald.ringVert,
      fragmentSource: GLEffectShaders.Emerald.ringFrag
    )
    ringPosition = glGetAttribLocation(ringProgram, "aPosition")
    ringColor = glGetAttribLocation(ringProgram, "aColor")
  }
  ...
  override func draw(trackData: GLTrackData, context: GLRenderContext, effectType: EffectType) {
    ...
    glUseProgram(vineProgram)
    glUniform1f(vineTime, time)
  ...
  private func drawVine(_ points: [GLTrailSample], context: GLRenderContext) {
    ...
    GLHelper.drawInterleaved(
      ...
      attributes: [
        GLVertexAttribute(location: vinePosition, size: 2, offsetBytes: 0),
        GLVertexAttribute(location: vineColor, size: 4, offsetBytes: 8),
        GLVertexAttribute(location: vineCenterDistance, size: 1, offsetBytes: 24),
        GLVertexAttribute(location: vineTrailDistance, size: 1, offsetBytes: 28),
      ],
  ...
  private func updateLeaves(context: GLRenderContext) {
  ...
  private func drawLeafQuads() {
    ...
    GLHelper.drawInterleaved(
      ...
      attributes: [
        GLVertexAttribute(location: leafPosition, size: 2, offsetBytes: 0),
        GLVertexAttribute(location: leafUV, size: 2, offsetBytes: 8),
        GLVertexAttribute(location: leafColor, size: 4, offsetBytes: 16),
      ],
  ...
  private func emitLeafQuad(_ leaf: Leaf) {
    ...
    let alpha = leaf.alpha.glClamped()
  ...
  private func drawTreeRings(_ trackData: GLTrackData, context: GLRenderContext) {
    let minDimension = context.minDimension
    guard minDimension > 0 else { return }
    ringFloats.clear()
    var vertexCount = 0
    ...
    for (_, points) in trackData where !points.isEmpty && heads < Self.maxHeads {
      ...
      let color = GLHelper.rgba(point.color)
    ...
    GLHelper.drawInterleaved(
      ...
      attributes: [
        GLVertexAttribute(location: ringPosition, size: 2, offsetBytes: 0),
        GLVertexAttribute(location: ringColor, size: 4, offsetBytes: 8),
      ],
      mode: GLenum(GL_LINES),
