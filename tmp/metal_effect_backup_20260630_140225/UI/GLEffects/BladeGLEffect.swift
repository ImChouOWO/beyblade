...
/// Port of BladeGLEffect.kt: double-helix blade, metal sparks, cracks, glints and sword-qi waves.
final class BladeGLEffect: GLEffect {
  private var program: GLuint = 0
  private var posLoc: GLint = -1
  private var colorLoc: GLint = -1
  private var distLoc: GLint = -1
  ...
  private let sparkFloats = GLFloatBuffer(capacity: 28 * 6 * 7)
  ...
  override func onGLReady(context: GLRenderContext) {
    program = GLHelper.buildProgram(
      vertexSource: GLEffectShaders.Blade.bladeVert,
      fragmentSource: GLEffectShaders.Blade.bladeFrag
    )
    posLoc = glGetAttribLocation(program, "aPosition")
    colorLoc = glGetAttribLocation(program, "aColor")
    distLoc = glGetAttribLocation(program, "aCenterDist")
  }
  ...
  override func draw(trackData: GLTrackData, context: GLRenderContext, effectType: EffectType) {
    ...
    for (_, points) in trackData where points.count >= 3 { drawBlade(points, context: context) }
  ...
  private func drawBlade(_ points: [GLTrailSample], context: GLRenderContext) {
    ...
    let base = GLHelper.vivid(points.last!.first.color)
    let twin = GLHelper.hueShift(base, degrees: 38)
    for strand in 0..<2 {
      let color = GLHelper.rgba(strand == 0 ? base : twin)
      ...
      for j in 0..<m {
        ...
        let whiteMix = ((u - 0.75) / 0.25).glClamped()
      ...
      drawBladeBuffer(context.ribbonFloats, mode: GLenum(GL_TRIANGLE_STRIP), count: m * 2)
    }
  }
  ...
  private func spawnFromTrack(_ trackData: GLTrackData, context: GLRenderContext) {
    ...
    for (trackID, points) in trackData where points.count >= 2 {
      ...
      if normalized > 0.009 { emitFromBody(points, minDimension: minDimension, context: context) }
      if normalized > 0.013 {
        let vx = dx * width * 0.5 / context.dtScale
        let vy = dy * height * 0.5 / context.dtScale
        if Double.random(in: 0...1) > 0.3 { spawnSpark(x: x, y: y, vx: vx, vy: vy) }
        if Double.random(in: 0...1) > 0.5 { spawnSpark(x: x, y: y, vx: vx, vy: vy) }
  ...
  private func emitFromBody(
    _ points: [GLTrailSample], minDimension: Float, context: GLRenderContext
  ) {
    ...
    if Double.random(in: 0...1) > 0.45 { spawnGlint(x: x, y: y, minDimension: minDimension) }
  }
  ...
  private func spawnCrack(
    x1: Float, y1: Float, x2: Float, y2: Float, minDimension: Float, context: GLRenderContext
  ) {
  ...
  private func drawCracks(context: GLRenderContext) {
    let width = Float(context.viewWidth)
    let height = Float(context.viewHeight)
    context.ribbonFloats.clear()
    var count = 0
    for crack in cracks where crack.active {
      ...
      let length = max(hypot(dx, dy), 0.001)
      let nx = -dy / length * 1.2 / (width / 2)
      let ny = dx / length * 1.2 / (height / 2)
      let alpha = crack.alpha.glClamped() * 0.6
    ...
    if count > 0 { drawBladeBuffer(context.ribbonFloats, mode: GLenum(GL_TRIANGLES), count: count) }
  }

  private func updateSparks(context: GLRenderContext) {
    let width = Float(context.viewWidth)
    let height = Float(context.viewHeight)
  ...
  private func drawSparks(context: GLRenderContext) {
    let width = Float(context.viewWidth)
    let height = Float(context.viewHeight)
    sparkFloats.clear()
    var count = 0
    for spark in sparks where spark.active {
      ...
      let length = speed * 1.5 + 4
      let endX = spark.x + dx * length / (width * 0.5)
      let endY = spark.y + dy * length / (height * 0.5)
      let nx = -dy * 1.6 / (width * 0.5)
      let ny = dx * 1.6 / (height * 0.5)
      let alpha = spark.alpha.glClamped()
    ...
    if count > 0 { drawBladeBuffer(sparkFloats, mode: GLenum(GL_TRIANGLES), count: count) }
  }
  ...
  private func spawnWave(
    ...
    wave.alpha = 0.95
    let c = GLHelper.rgba(GLHelper.vivid(color))
    wave.r = c.0 * 0.75 + 0.25
    wave.g = c.1 * 0.75 + 0.25
    wave.b = c.2 * 0.75 + 0.25
  }

  private func updateWaves(context: GLRenderContext) {
    let width = Float(context.viewWidth)
    let height = Float(context.viewHeight)
    let dt = context.dtScale
  ...
  private func drawWaves(context: GLRenderContext) {
    let width = Float(context.viewWidth)
    let height = Float(context.viewHeight)
    let maxBand = min(width, height) * 0.008
    for wave in waves where wave.active {
      context.ribbonFloats.clear()
      for i in 0...12 {
        let t = Float(i) / 12
        ...
        let inner = max(wave.radiusPx - band, 0)
        let outer = wave.radiusPx + band
        let cosine = cos(angle)
        let sine = sin(angle)
        let alpha = wave.alpha.glClamped()
      ...
      drawBladeBuffer(context.ribbonFloats, mode: GLenum(GL_TRIANGLE_STRIP), count: 26)
    }
  }
  ...
  private func drawGlints(context: GLRenderContext) {
    let width = Float(context.viewWidth)
    let height = Float(context.viewHeight)
    context.ribbonFloats.clear()
    var count = 0
    for glint in glints where glint.active {
      ...
      let armWidth = glint.sizePx * 0.16
      for arm in 0..<2 {
        let angle = glint.angle + Float(arm) * .pi / 2
        let dx = cos(angle)
        let dy = sin(angle)
        let lx = dx * glint.sizePx / (width / 2)
        let ly = dy * glint.sizePx / (height / 2)
  ...
  private func drawBladeBuffer(_ buffer: GLFloatBuffer, mode: GLenum, count: Int) {
    glUseProgram(program)
    GLHelper.drawInterleaved(
      ...
      attributes: [
        ...
        GLVertexAttribute(location: distLoc, size: 1, offsetBytes: 24),
