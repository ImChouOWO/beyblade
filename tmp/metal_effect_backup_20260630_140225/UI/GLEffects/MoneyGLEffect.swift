...
/// Port of MoneyGLEffect.kt (`IronShieldGLEffect`): gold ribbon, coins, sparks and impact rings.
final class MoneyGLEffect: GLEffect {
  private var goldProgram: GLuint = 0
  private var goldPosLoc: GLint = -1
  private var goldColorLoc: GLint = -1
  private var goldDistLoc: GLint = -1

  private var coinProgram: GLuint = 0
  private var coinPosLoc: GLint = -1
  private var coinUVLoc: GLint = -1
  private var coinColorLoc: GLint = -1
  ...
  private let coinFloats = GLFloatBuffer(capacity: 36 * 6 * 8)
  private let sparkFloats = GLFloatBuffer(capacity: 48 * 6 * 7)
  ...
  override func onGLReady(context: GLRenderContext) {
    goldProgram = GLHelper.buildProgram(
      vertexSource: GLEffectShaders.Money.goldVert,
      fragmentSource: GLEffectShaders.Money.goldFrag
    )
    goldPosLoc = glGetAttribLocation(goldProgram, "aPosition")
    goldColorLoc = glGetAttribLocation(goldProgram, "aColor")
    goldDistLoc = glGetAttribLocation(goldProgram, "aCenterDist")

    coinProgram = GLHelper.buildProgram(
      vertexSource: GLEffectShaders.Money.coinVert,
      fragmentSource: GLEffectShaders.Money.coinFrag
    )
    coinPosLoc = glGetAttribLocation(coinProgram, "aPosition")
    coinUVLoc = glGetAttribLocation(coinProgram, "aUV")
    coinColorLoc = glGetAttribLocation(coinProgram, "aColor")
  }
  ...
  private func drawRibbon(_ points: [GLTrailSample], context: GLRenderContext) {
    ...
    drawGold(buffer: context.ribbonFloats, mode: GLenum(GL_TRIANGLE_STRIP), count: n * 2)
  }
  ...
  private func updateCoins(context: GLRenderContext) {
  ...
  private func drawCoins() {
    ...
    for coin in coins where coin.active {
      ...
      let alpha = coin.alpha.glClamped()
    ...
    GLHelper.drawInterleaved(
      ...
      strideBytes: 32,
      attributes: [
        GLVertexAttribute(location: coinPosLoc, size: 2, offsetBytes: 0),
        GLVertexAttribute(location: coinUVLoc, size: 2, offsetBytes: 8),
        GLVertexAttribute(location: coinColorLoc, size: 4, offsetBytes: 16),
      ],
  ...
  private func drawSparks() {
    ...
    drawGold(buffer: sparkFloats, mode: GLenum(GL_TRIANGLES), count: vertexCount)
  }
  ...
  private func drawRings(context: GLRenderContext) {
  ...
  private func drawRadialBand(
    ...
    drawGold(
      buffer: context.ribbonFloats, mode: GLenum(GL_TRIANGLE_STRIP), count: (segments + 1) * 2)
  }

  private func drawGold(buffer: GLFloatBuffer, mode: GLenum, count: Int) {
    glUseProgram(goldProgram)
    GLHelper.drawInterleaved(
      buffer: buffer,
      strideBytes: 28,
      attributes: [
        GLVertexAttribute(location: goldPosLoc, size: 2, offsetBytes: 0),
        GLVertexAttribute(location: goldColorLoc, size: 4, offsetBytes: 8),
        GLVertexAttribute(location: goldDistLoc, size: 1, offsetBytes: 24),
      ],
      mode: mode,
      vertexCount: count
    )
