...
enum EffectType: String, CaseIterable, Identifiable {
    ...
    /// 只作為 StoreKit 商品尚未完成載入時的 UI 備註。
    /// 正式價格必須使用 StoreKit 的 Product.displayPrice。
    var fallbackPrice: Int {
        isDefaultOwned ? 0 : 40
    }

    /// 相容原本程式可能使用的 effect.price。
    var price: Int {
        fallbackPrice
    }
    ...
    ///
    /// 不能用此屬性判斷使用者目前是否已解鎖，
    /// 真正權限必須呼叫 EffectPurchaseStore.isPurchased(_:)。
    var isLocked: Bool {
        requiresPurchase
    }

    // MARK: - StoreKit Product ID

    /// 每個單一付費特效對應的 StoreKit Product ID。
    ///
    /// 免費特效回傳 nil，因此不會被送到 StoreKit 載入。
    ...
    /// 全部特效禮包的 StoreKit Product ID。
    static let premiumPackProductID =
        StoreProductID.premiumPack

    /// 所有需要從 StoreKit 載入的商品 ID。
    ///
    /// 包含：
    /// - 9 個單一特效
    /// - 1 個全部特效禮包
    static var allProductIDs: Set<String> {
        let individualIDs = shopEffects.compactMap(\.productID)

        return Set(individualIDs)
            .union([premiumPackProductID])
    }

    /// 只有單一特效商品的 Product ID，不包含禮包。
    ...
    var colorOverride: UIColor? {
        switch self {
        case .lightning:
            return UIColor(hex: 0xFFDD22)

        case .fire:
            return UIColor(hex: 0xFF2A12)

        case .stardust:
            return nil

        case .wave:
            return UIColor(hex: 0x168CFF)

        case .thunder:
            return UIColor(hex: 0xFFD34E)

        case .vortex:
            return UIColor(hex: 0xD8E2EA)

        case .dark:
            return UIColor(hex: 0x9DEBFF)

        case .crimson:
            return UIColor(hex: 0xC9082E)

        case .deathRay:
            return UIColor(hex: 0xA54CFF)

        case .emerald:
            return UIColor(hex: 0x24D77A)
...
/// `.storekit` 與 App Store Connect 的 Product ID
/// 必須與以下字串完全相同。
