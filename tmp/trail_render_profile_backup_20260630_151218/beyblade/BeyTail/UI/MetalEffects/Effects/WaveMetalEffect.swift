...
final class WaveMetalEffect: MetalEffect {
  ...
  private func drawFluidRibbon(_ points: [MetalTrailSample], context: MetalRenderContext) {
    ...
    for i in 0..<n {
      ...
      let halfWidth =
        Self.baseTrailHalfWidth
        * Self.trailWidthMultiplier
        * (0.35 + 0.65 * alpha)
  ...
  private func spawnParticles(trackData: MetalTrackData, context: MetalRenderContext) {
    for (trackID, points) in trackData where points.count >= 2 {
      ...
      if normalizedMovement > Self.minimumRippleMovement {
        ...
        if elapsed >= Self.rippleIntervalSeconds {
          spawnRipple(x: x, y: y)
          rippleElapsedByTrack[trackID] =
            elapsed - Self.rippleIntervalSeconds
        } else {
          rippleElapsedByTrack[trackID] = elapsed
        }
      }

      guard distance > 0.008, Double.random(in: 0...1) > 0.3 else {
        continue
      }
  ...
  private static let trailWidthMultiplier: Float = 1.25
