...
final class IceShatterMetalEffect: MetalEffect {
  ...
  private func spawnFrostAndFog(_ trackData: MetalTrackData, context: MetalRenderContext) {
    ...
    for (_, points) in trackData where !points.isEmpty {
      ...
      if points.count >= 3 {
        let frostCount = context.particleEmissionCount(
          baseCount: Self.frostPerFrame * context.dtScale
        )
        for _ in 0..<frostCount {
      ...
      if context.shouldSpawnParticle(
        baseProbability: Self.fogRate * context.dtScale
      ) {
  ...
  private func spawnFromTrack(_ trackData: MetalTrackData, context: MetalRenderContext) {
    ...
    for (trackID, points) in trackData where points.count >= 2 {
      ...
      if normalized > 0.007 {
        ...
        if context.shouldSpawnParticle(baseProbability: chance) {
  ...
  private func spawnShardsFromTrail(
    ...
    let targetShardCount = context.particleEmissionCount(baseCount: 6)
    guard targetShardCount > 0 else { return }
    var spawned = 0
    for shard in shards where !shard.active {
      ...
      spawned += 1
      if spawned >= targetShardCount { break }
    }
  }
  ...
  private func drawShards(context: MetalRenderContext) {
    ...
    for shard in shards where shard.active {
      ...
      func vertex(_ index: Int) -> (Float, Float) {
        let rx =
          (shard.offsetX[index] * cosine - shard.offsetY[index] * sine)
          * particleSizeScale
        let ry =
          (shard.offsetX[index] * sine + shard.offsetY[index] * cosine)
          * particleSizeScale
