...
/// Direct Swift/Metal port of InkWashMetalEffect.kt.
/// Keeps the three ink strands, dry-brush shader, droplets and dissolve behavior.
final class InkWashMetalEffect: MetalEffect {
  ...
  private var inkProgram: MetalProgramID = 0
  private var inkPosition: MetalLocation = -1
  private var inkColor: MetalLocation = -1
  private var inkCenterDistance: MetalLocation = -1
  private var inkTrailDistance: MetalLocation = -1
  private var inkTime: MetalLocation = -1
  private var inkStrandAlpha: MetalLocation = -1

  private var dropProgram: MetalProgramID = 0
  private var dropPosition: MetalLocation = -1
  private var dropColor: MetalLocation = -1
  private var dropSize: MetalLocation = -1
  private var dropDissolve: MetalLocation = -1
  ...
  private let dropFloats = MetalFloatBuffer(capacity: 32 * 8)
  ...
  override func onMetalReady(context: MetalRenderContext) {
    inkProgram = MetalHelper.makeProgram(.ink)
    inkPosition = metalGetAttribLocation(inkProgram, "aPosition")
    inkColor = metalGetAttribLocation(inkProgram, "aColor")
    inkCenterDistance = metalGetAttribLocation(inkProgram, "aCenterDist")
    inkTrailDistance = metalGetAttribLocation(inkProgram, "aTrailDist")
    inkTime = metalGetUniformLocation(inkProgram, "uTime")
    inkStrandAlpha = metalGetUniformLocation(inkProgram, "uStrandAlpha")

    dropProgram = MetalHelper.makeProgram(.inkDrop)
    dropPosition = metalGetAttribLocation(dropProgram, "aPosition")
    dropColor = metalGetAttribLocation(dropProgram, "aColor")
    dropSize = metalGetAttribLocation(dropProgram, "aSize")
    dropDissolve = metalGetAttribLocation(dropProgram, "aDissolve")
  }
  ...
  override func draw(trackData: MetalTrackData, context: MetalRenderContext, effectType: EffectType) {
    ...
    metalUseProgram(inkProgram)
    metalUniform1f(inkTime, time)
  ...
  private func drawInkStrand(
    ...
    let color = MetalHelper.rgba(points[n - 1].first.color)
    let isMain = strand == 0
    let baseHalfWidth =
    ...
    metalUniform1f(inkStrandAlpha, isMain ? 1 : 0.45)
    ...
    MetalHelper.drawInterleaved(
      ...
      attributes: [
        MetalVertexAttribute(location: inkPosition, size: 2, offsetBytes: 0),
        MetalVertexAttribute(location: inkColor, size: 4, offsetBytes: 8),
        MetalVertexAttribute(location: inkCenterDistance, size: 1, offsetBytes: 24),
        MetalVertexAttribute(location: inkTrailDistance, size: 1, offsetBytes: 28),
      ],
  ...
  private func spawnDrops(_ trackData: MetalTrackData, context: MetalRenderContext) {
    let minDimension = context.minDimension
    guard minDimension > 0 else { return }
    for (_, points) in trackData where points.count >= 3 {
      guard context.shouldSpawnParticle(
        baseProbability: Self.dropRate * context.dtScale
      ) else { continue }
  ...
  private func updateDrops(context: MetalRenderContext) {
  ...
  private func drawDrops() {
    ...
    var count = 0
    for drop in drops where drop.active {
      let dissolve = (drop.age / drop.life).metalClamped()
    ...
    metalUseProgram(dropProgram)
    MetalHelper.drawInterleaved(
      buffer: dropFloats,
      strideBytes: Self.bytesPerDrop,
      attributes: [
        MetalVertexAttribute(location: dropPosition, size: 2, offsetBytes: 0),
        MetalVertexAttribute(location: dropColor, size: 4, offsetBytes: 8),
        MetalVertexAttribute(location: dropSize, size: 1, offsetBytes: 24),
        MetalVertexAttribute(location: dropDissolve, size: 1, offsetBytes: 28),
      ],
      mode: MGL_POINTS,
      vertexCount: count
    )
