...
/// Direct Swift/Metal port of CrimsonLotusMetalEffect.kt.
final class CrimsonLotusMetalEffect: MetalEffect {
  ...
  private var fireProgram: MetalProgramID = 0
  private var firePosition: MetalLocation = -1
  private var fireColor: MetalLocation = -1
  private var fireCenterDistance: MetalLocation = -1
  private var fireTrailDistance: MetalLocation = -1
  private var fireTime: MetalLocation = -1

  private var polygonProgram: MetalProgramID = 0
  private var polygonPosition: MetalLocation = -1
  private var polygonColor: MetalLocation = -1
  private var polygonCenterDistance: MetalLocation = -1

  private var hazeProgram: MetalProgramID = 0
  private var hazePosition: MetalLocation = -1
  private var hazeColor: MetalLocation = -1
  private var hazeSize: MetalLocation = -1
  ...
  private let polygonFloats = MetalFloatBuffer(capacity: 16 * 18 * 7)
  private let emberFloats = MetalFloatBuffer(capacity: 24 * 7)
  private let hazeFloats = MetalFloatBuffer(capacity: 20 * 7)
  ...
  override func onMetalReady(context: MetalRenderContext) {
    fireProgram = MetalHelper.makeProgram(.crimsonFire)
    firePosition = metalGetAttribLocation(fireProgram, "aPosition")
    fireColor = metalGetAttribLocation(fireProgram, "aColor")
    fireCenterDistance = metalGetAttribLocation(fireProgram, "aCenterDist")
    fireTrailDistance = metalGetAttribLocation(fireProgram, "aTrailDist")
    fireTime = metalGetUniformLocation(fireProgram, "uTime")

    polygonProgram = MetalHelper.makeProgram(.crimsonPolygon)
    polygonPosition = metalGetAttribLocation(polygonProgram, "aPosition")
    polygonColor = metalGetAttribLocation(polygonProgram, "aColor")
    polygonCenterDistance = metalGetAttribLocation(polygonProgram, "aCenterDist")

    hazeProgram = MetalHelper.makeProgram(.crimsonHaze)
    hazePosition = metalGetAttribLocation(hazeProgram, "aPosition")
    hazeColor = metalGetAttribLocation(hazeProgram, "aColor")
    hazeSize = metalGetAttribLocation(hazeProgram, "aSize")
  }
  ...
  override func draw(trackData: MetalTrackData, context: MetalRenderContext, effectType: EffectType) {
    ...
    metalUseProgram(fireProgram)
    metalUniform1f(fireTime, time)
  ...
  private func drawFireTongues(
    ...
    for strand in 0..<2 {
      ...
      MetalHelper.drawInterleaved(
        ...
        attributes: [
          MetalVertexAttribute(location: firePosition, size: 2, offsetBytes: 0),
          MetalVertexAttribute(location: fireColor, size: 4, offsetBytes: 8),
          MetalVertexAttribute(location: fireCenterDistance, size: 1, offsetBytes: 24),
          MetalVertexAttribute(location: fireTrailDistance, size: 1, offsetBytes: 28),
        ],
    ...
    for (trackID, points) in trackData where points.count >= 2 {
      ...
      if points.count >= 4,
         context.shouldSpawnParticle(
          baseProbability: min(0.55 * context.dtScale, 1)
         ) {
      ...
      if normalized > 0.010, context.shouldSpawnParticle(baseProbability: 0.60) {
        spawnFireball(x: x, y: y, minDimension: minDimension, big: false, color: point.color)
      }
      if normalized > 0.016 {
        for _ in 0..<context.particleEmissionCount(baseCount: 3) {
  ...
  private func updateFireballs(context: MetalRenderContext) {
    let width = Float(context.viewWidth)
    let height = Float(context.viewHeight)
  ...
  private func drawFireballs(context: MetalRenderContext) {
    let width = Float(context.viewWidth)
    let height = Float(context.viewHeight)
    polygonFloats.clear()
    var vertexCount = 0
    for fireball in fireballs where fireball.active {
      let alpha = fireball.alpha.metalClamped()
      let scale =
        fireball.scale * context.particleSizeMultiplier
        * Float.random(in: 0.94...1.06)
      for index in 0..<fireball.vertexCount {
    ...
    metalUseProgram(polygonProgram)
    MetalHelper.drawInterleaved(
      buffer: polygonFloats,
      strideBytes: Self.bytesPerPoint,
      attributes: [
        MetalVertexAttribute(location: polygonPosition, size: 2, offsetBytes: 0),
        MetalVertexAttribute(location: polygonColor, size: 4, offsetBytes: 8),
        MetalVertexAttribute(location: polygonCenterDistance, size: 1, offsetBytes: 24),
      ],
  ...
  private func updateEmbers(context: MetalRenderContext) {
    let width = Float(context.viewWidth)
    let height = Float(context.viewHeight)
    let dt = context.dtScale
  ...
  private func drawEmbers() {
    ...
    var count = 0
    for ember in embers where ember.active {
      let alpha = (ember.alpha * Float.random(in: 0.6...1)).metalClamped()
  ...
  private func fillHazeBuffer() -> Int {
    ...
    var count = 0
    for haze in hazes where haze.active {
      hazeFloats.put(haze.x).put(haze.y).put(1).put(0.96).put(0.90).put(haze.alpha.metalClamped()).put(
        haze.sizePx)
      count += 1
    }
    return count
  ...
  private func drawPointPool(_ buffer: MetalFloatBuffer, count: Int) {
    guard count > 0 else { return }
    metalUseProgram(hazeProgram)
    MetalHelper.drawInterleaved(
      buffer: buffer,
      strideBytes: Self.bytesPerPoint,
      attributes: [
        MetalVertexAttribute(location: hazePosition, size: 2, offsetBytes: 0),
        MetalVertexAttribute(location: hazeColor, size: 4, offsetBytes: 8),
        MetalVertexAttribute(location: hazeSize, size: 1, offsetBytes: 24),
      ],
