...
final class DeathRayMetalEffect: MetalEffect {
  ...
  private func spawnVortex(_ trackData: MetalTrackData, context: MetalRenderContext) {
    ...
    for (_, points) in trackData where !points.isEmpty {
      ...
      let vortexCount = context.particleEmissionCount(
        baseCount: Self.vortexPerFrame * context.dtScale
      )
      for _ in 0..<vortexCount {
        spawnOneVortex(
  ...
  private func drawCoreAndArcs(_ trackData: MetalTrackData, context: MetalRenderContext) {
    ...
    for (_, points) in trackData where !points.isEmpty && headCount < Self.maxHeads {
      ...
      let arcCount = context.particleEmissionCount(
        baseCount: Float(Self.arcsPerHead)
      )
      for _ in 0..<arcCount {
        ...
        let segmentLength = context.scaledParticleSize(
          minDimension * Float.random(in: 0.018...0.038)
        )
    ...
    metalLineWidth(context.scaledParticleSize(2))
    MetalHelper.drawInterleaved(
      buffer: arcFloats,
  ...
  private func spawnHaze(_ trackData: MetalTrackData, context: MetalRenderContext) {
    ...
    hazeAccumulator +=
      context.scaledParticleRate(Self.hazeSpawnRate) * context.dtScale
