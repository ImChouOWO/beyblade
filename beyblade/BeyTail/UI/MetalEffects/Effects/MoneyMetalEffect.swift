...
final class MoneyMetalEffect: MetalEffect {
  ...
  private func spawnFromTrack(_ trackData: MetalTrackData, context: MetalRenderContext) {
    ...
    for (trackID, points) in trackData where points.count >= 2 {
      ...
      if normalized > 0.008 {
        let count = context.particleEmissionCount(
          baseCount: Float(1 + min(Int(normalized * 70), 2))
        )
        ...
        if context.shouldSpawnParticle(baseProbability: 0.60) {
          spawnSpark(x: x, y: y, angle: movementAngle + .pi, coneHalf: 0.4, count: 1, context: context)
        }
      }

      if normalized > 0.018 {
        let back = movementAngle + Float.pi
        let burst = context.particleEmissionCount(
          baseCount: Float(Int.random(in: 3...5))
        )
        for _ in 0..<burst {
          spawnCoin(
            x: x, y: y,
        ...
        spawnSpark(x: x, y: y, angle: back, coneHalf: 0.7, count: 3, context: context)
      }
    }
  }
  ...
  private func drawCoins(context: MetalRenderContext) {
    ...
    for coin in coins where coin.active {
      ...
      let hx = coin.sizePx * 0.5 * flipScale * sizeScale
      let hy = coin.sizePx * 0.5 * sizeScale
  ...
  private func spawnSpark(
    x: Float, y: Float, angle: Float, coneHalf: Float, count: Int,
    context: MetalRenderContext
  ) {
    let targetCount = context.particleEmissionCount(baseCount: Float(count))
  ...
  private func drawSparks(context: MetalRenderContext) {
    ...
    for spark in sparks where spark.active {
      ...
      let sparkHalfWidth = context.scaledParticleSize(spark.halfWidthPx)
      let nx = -dy * sparkHalfWidth / halfWidth
      let ny = dx * sparkHalfWidth / halfHeight
  ...
  private func drawRings(context: MetalRenderContext) {
    ...
    for ring in rings where ring.active {
      drawRadialBand(
        ...
        halfBandPx: context.scaledParticleSize(minDimension * 0.0034),
        segments: 24,
