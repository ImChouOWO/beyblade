...
final class EmeraldMetalEffect: MetalEffect {
  ...
  private func spawnFromTrack(_ trackData: MetalTrackData, context: MetalRenderContext) {
    ...
    for (_, points) in trackData where points.count >= 4 {
      ...
      guard context.shouldSpawnParticle(
        baseProbability: 0.5 * context.dtScale
      ) else { continue }
  ...
  private func drawLeafQuads(context: MetalRenderContext) {
    leafFloats.clear()
    var count = 0
    for leaf in leaves where leaf.active {
      emitLeafQuad(leaf, sizeScale: context.particleSizeMultiplier)
      count += 1
    }
  ...
  private func emitLeafQuad(_ leaf: Leaf, sizeScale: Float) {
    let cosine = cos(leaf.angle)
    let sine = sin(leaf.angle)
    let halfLength = leaf.lengthPx * 0.5 * sizeScale
    let halfWidth = leaf.widthPx * 0.5 * sizeScale
