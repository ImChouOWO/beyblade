...
final class SprayPaintMetalEffect: MetalEffect {
  ...
  private func spawnFromTrack(_ trackData: MetalTrackData, context: MetalRenderContext) {
    ...
    for (trackID, points) in trackData where points.count >= 2 {
      ...
      let mistCount = context.particleEmissionCount(
        baseCount: Self.mistPerFrame * context.dtScale
      )
      ...
      if normalizedMovement > 0.010, context.shouldSpawnParticle(baseProbability: 0.55) {
      ...
      if normalizedMovement > 0.018 {
        ...
        for _ in 0..<context.particleEmissionCount(baseCount: 2) {
          spawnSplat(
