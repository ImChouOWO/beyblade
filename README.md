...
> StoreKit 購買權限與試用限制已完成串接；各付費特效的完整視覺效果可依上述規格持續擴充。

## StoreKit 2 特效商店

付費特效使用 StoreKit 2 的非消耗型商品。

商品 ID 建議依照 App 的 Bundle Identifier 建立：

```text
<bundle-id>.effect.<effect-id>
<bundle-id>.effects.<pack-id>
```

商品 ID 模板：

```text
<bundle-id>.effect.wave
<bundle-id>.effect.steel_shield
<bundle-id>.effect.blade_dance
<bundle-id>.effect.ice_break
<bundle-id>.effects.premium_pack
```

其中：

- `<bundle-id>`：App 的 Bundle Identifier
- `<effect-id>`：單一特效的唯一識別名稱
- `<pack-id>`：特效組合包的唯一識別名稱

商品對應如下：

| Product ID 模板 | 商品 |
|---|---|
| `<bundle-id>.effect.wave` | 滔天浪潮 |
| `<bundle-id>.effect.steel_shield` | 不滅鋼盾 |
| `<bundle-id>.effect.blade_dance` | 爆刃亂舞 |
| `<bundle-id>.effect.ice_break` | 狂暴冰裂 |
| `<bundle-id>.effects.premium_pack` | 四款付費特效組合包 |

> 實際上架前，必須將 `<bundle-id>` 替換成 Xcode 專案中的 Bundle Identifier，並確保程式碼、StoreKit Configuration File 與 App Store Connect 使用完全相同的 Product ID。

購買紀錄由 Apple Account 與 StoreKit 管理，不需要另外建立使用者登入系統。

App 啟動後會透過 `Transaction.currentEntitlements` 檢查目前有效的購買權限。
...
- [ ] 確認正式 App Bundle Identifier
- [ ] 完成 Apple Developer Program 設定
- [ ] 完成付費 App 合約
- [ ] 完成銀行與稅務資料
- [ ] 建立 `<bundle-id>.effect.wave`
- [ ] 建立 `<bundle-id>.effect.steel_shield`
- [ ] 建立 `<bundle-id>.effect.blade_dance`
- [ ] 建立 `<bundle-id>.effect.ice_break`
- [ ] 建立 `<bundle-id>.effects.premium_pack`
- [ ] 將所有商品類型設為 Non-Consumable
- [ ] 設定商品名稱與繁體中文描述
- [ ] 設定各商品價格級距
- [ ] 上傳 App 內購買審核截圖
