  ...
  private func drawFluidRibbon(_ points: [MetalTrailSample], context: MetalRenderContext) {
    ...
    for i in 0..<n {
      ...
      if i == 0 {
        ...
        normal = MetalHelper.segNormal(pointX[i - 1], pointY[i - 1], x, y)
      } else {
  ...
  private func spawnParticles(trackData: MetalTrackData, context: MetalRenderContext) {
    for (trackID, points) in trackData where points.count >= 2 {
      ...
      for particle in particles where !particle.active {
        ...
        let strength = (normalizedMovement * 1.5).metalClamped(0.02, 0.045)
  ...
  private func drawParticles() {
    ...
    for particle in particles where particle.active {
      ...
      particleFloats
        ...
        .put(r).put(g).put(1).put(particle.alpha.metalClamped()).put(particle.sizePx)
      count += 1
    }
